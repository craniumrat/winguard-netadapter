...
#pragma NDIS_INIT_FUNCTION(winguardntkmdfRegisterDevice)


_IRQL_requires_max_(PASSIVE_LEVEL)
NDIS_STATUS
winguardntkmdfRegisterDevice(
    VOID
    )
{
    ...
    DEBUGP(DL_TRACE, "==>winguardntkmdfRegisterDevice\n");
   
    
    NdisZeroMemory(DispatchTable, (IRP_MJ_MAXIMUM_FUNCTION+1) * sizeof(PDRIVER_DISPATCH));
    
    DispatchTable[IRP_MJ_CREATE] = winguardntkmdfDispatch;
    DispatchTable[IRP_MJ_CLEANUP] = winguardntkmdfDispatch;
    DispatchTable[IRP_MJ_CLOSE] = winguardntkmdfDispatch;
    DispatchTable[IRP_MJ_DEVICE_CONTROL] = winguardntkmdfDeviceIoControl;
    ...
    DEBUGP(DL_TRACE, "<==winguardntkmdfRegisterDevice: %x\n", Status);
...
_IRQL_requires_max_(PASSIVE_LEVEL)
VOID
winguardntkmdfDeregisterDevice(
    VOID
    )
